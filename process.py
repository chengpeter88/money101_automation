    ...
    try:
        ...
        for partner, keywords in partner_keywords.items():
            ...
            for keyword in keywords:
                # 使用正則表達式找出每個關鍵詞的出現次數
                mentions += len(re.findall(r'\b' + re.escape(keyword) + r'\b', content, re.IGNORECASE))
...
# 按文章數量排序
sorted_partners = sorted(partner_article_counts.items(), key=lambda x: x[1], reverse=True)
print("\n合作夥伴出現在文章的次數:")
...
output = {
    ...
    } for r in results if r['partners']]
}
